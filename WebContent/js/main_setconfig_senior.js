...
function createPageContent(parent,data,num,pagination){
	...
	var keyIndex='';
	/**
	 * 高级配置列表中，鼠标悬停时显示提示框，显示时间为3秒，之后会自动消失
	 */
	...
	$(".preview_btn").click(function(){
		...
		if(_thisType=='image_list'){
			...
			var _thisHtml='<div><form action="" method="post">\
				<div>\
					<textarea name="bbcode_field" id="bbcode_field_box" style="height:300px;width:600px;">'+_thisValue+'</textarea>\
				</div>\
			</form>\
			<div class="senior_btn_group clearfix">\
			    <input dataType='+_thisType+' configKey='+_thisKey+' ownerId='+_thisOwnerId+' priority='+_thisPri+' id="senior_delete_btn" type="button" value="使用默认值"/><input dataType='+_thisType+' configKey='+_thisKey+' ownerId='+_thisOwnerId+' priority='+_thisPri+' id="senior_save_btn" type="button" value="保存"/>\
		    </div></div>';
		}else{
			var _thisHtml='\
			<div class="senior_edit_box">'+_thisHtmlPath+'\
		...
		$("#senior_save_btn").click(function(){
			...
			if(_thisType=="list"){
				...
				printTimer=setTimeout(function(){
					...
					editSubmitString(_this,_thisPrintValue,function(){
						if(subResult.message=="OK"){
							layer.alert("修改保存成功",{
								icon:1,
								yes:function(index){
...
function searchDown(level,callback){
	...
	var searchHost='';
	if(priority==7){
		searchHost="https://"+host+"/signalmaster/rest/config/advance/"+ownerId+"/255";
	}else{
		searchHost="https://"+host+"/signalmaster/rest/config/advance/"+ownerId+"/"+priority;
	}
