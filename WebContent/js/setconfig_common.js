...
$(document).ready(function(){
	...
	//加载完成显示全部
	var groupUrl="https://"+host+"/signalmaster/rest/config/main/"+groupId+"/3/null";
	...
	createRobotsUser(robotList,function(){
		...
		$("#robot_group ul li").click(function(){
			...
			changeRobots("https://"+host+"/signalmaster/rest/config/main/"+($(this).attr("robotId"))+"/255/null");	
		})
		/*切换组*/
		$("#robot_group").on("click",".robot_group_title",function(){
			var _thisSceneId=$(this).attr("sceneId");
			window.localStorage.bookPos=_thisSceneId;
			layerLoad();
			$(this).parent().addClass("robot_group_box_current").siblings().removeClass("robot_group_box_current");
			$(".all_robots").removeClass("all_robots_current");
			$("#robot_group").find("li").removeClass("robotCurrent");
			if(_thisSceneId==0){
				changeRobots("https://"+host+"/signalmaster/rest/config/main/"+groupId+"/3/null")
			}else{
				changeRobots("https://"+host+"/signalmaster/rest/config/main/"+_thisSceneId+"/5/null");
			}	
		})
		...
		$(".robot_edit_model_save").click(function(){
			/*$(".add_scene").off("click");
			$("#add_scene_box").addClass("displayNone");
			$(".delete_scene").addClass("displayNone");
			$(this).addClass("displayNone");
			$(".robot_edit_model").removeClass("displayNone");
			$("#add_scene_box").addClass("displayNone");
			$(".robot_group_title").removeClass("displayNone");
			$(".scene_title_edit").addClass("displayNone");
			$("#robot_group ul li").css("cursor","pointer");
			for(var i=0;i<sortableList.length;i++){
				sortableList[i].destroy();
			}
			sortableList=[];*/
			location.reload();
		})
