    ...
    packageList(packageUrl,function(){
    	...
    	for(var i=0;i<data.length;i++){
   		...
   		$("#setconfig_qiye").change(function(){
			...
			//机器人更新
			createSelect(getRobotUrl,$("#setconfig_robots"),function(){
				var allData=logList();
				console.log(allData)
				layerLoad();
...
function logList(){
	checkTime();
	var searchId=getCurrentRobot().groupId;
	var groupFlag=getCurrentRobot().groupFlag;
...
function getCurrentRobot(){
	...
	var groupFlag=0;
	if(currentRobot=="null" && currentGroup==0){
		groupFlag=0;
	}
	if(currentRobot!="null"){
		groupId=currentRobot;
		groupFlag=-1;
	}
	if(currentGroup!=0 && currentRobot=="null"){
		groupId=currentGroup;
		groupFlag=1;
	}
	return {groupId:groupId,groupFlag:groupFlag};
}
